...
export async function PUT(request: NextRequest) {
  try {
    ...
    const timezone: string = (typeof body.timezone === 'string' && body.timezone.trim()) ? body.timezone : 'EST'
    const companySize: string = (typeof body.company_size === 'string' && body.company_size.trim()) ? body.company_size : 'startup'
    const maxMeetingsPerWeek: string = (typeof body.max_meetings_per_week === 'string' && body.max_meetings_per_week.trim()) ? body.max_meetings_per_week : '1'
