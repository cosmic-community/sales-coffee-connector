...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Fix TypeScript errors by ensuring string types with guaranteed string values
    const timezone: string = (body.timezone && typeof body.timezone === 'string') ? body.timezone : 'EST'
    const companySize: string = (body.company_size && typeof body.company_size === 'string') ? body.company_size : 'startup'
    const maxMeetingsPerWeek: string = (body.max_meetings_per_week && typeof body.max_meetings_per_week === 'string') ? body.max_meetings_per_week : '1'
